...
class Node(ScatterLayout):
	...
	def set_bool(self, ins, _name, val, button, tf_list):
		tf_list.select(button.text)
		self.properties[_name] = val
	...
	def set_val(self, obj, val, _name):
		try:
			if val != '':
				if self.properties[_name][0] == int:
					self.properties[_name][1] = int(val)

				elif self.properties[_name][0] == float:
					self.properties[_name][1] = float(int(val))

				# elif self.properties[_name][0] == bool:
				# 	self.properties[_name][1] = bool(val)
