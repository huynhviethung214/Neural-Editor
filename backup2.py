...
class Node(ScatterLayout):
	...
	def add_drop_down_list(self):
		...
		self.drop_butt = Button(text='Hidden Layer',
						   		width=self.width - 14,
						   		height=self.ci_height)

		self.drop_butt.bind(on_release=self.drop_list.open)
		self.drop_list.bind(on_select=lambda instance, x: setattr(self.drop_butt, 'text', x))
